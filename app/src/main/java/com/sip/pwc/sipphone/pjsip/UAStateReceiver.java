...
package com.sip.pwc.sipphone.pjsip;
...
import com.sip.pwc.sipphone.service.SipNotifications;
import com.sip.pwc.sipphone.service.SipService;
import com.sip.pwc.sipphone.service.impl.SipCallSessionImpl;
...
import com.sip.pwc.sipphone.utils.CallLogHelper;
import com.sip.pwc.sipphone.utils.Compatibility;
import com.sip.pwc.sipphone.utils.Threading;
...
public class UAStateReceiver extends Callback {
                ...
                for (SipCallSessionImpl existingCall : calls) {
                    if (!existingCall.isAfterEnded() && existingCall.getCallId() != callId) {
                        if (!pjService.service.supportMultipleCalls) {
                            ...
                            // multiple calls
                            // Send busy here
                            pjsua.call_hangup(callId, SipCallSession.StatusCode.BUSY_HERE, null, null);
                            unlockCpu();
                            return;
                        } else {
    ...
    public void on_call_tsx_state(int call_id, org.pjsip.pjsua.SWIGTYPE_p_pjsip_transaction tsx, pjsip_event e) {
        ...
        try {
            ...
            Log.d(THIS_FILE, "Call TSX state >>");
        } catch (SipService.SameThreadException ex) {
            // We don't care about that we are at least in a pjsua thread
        } finally {
            // Unlock CPU anyway
            unlockCpu();
    ...
    public void on_buddy_state(int buddyId) {
        ...
        SipManager.PresenceStatus presStatus = SipManager.PresenceStatus.UNKNOWN;
        ...
        switch (binfo.getStatus()) {
            case PJSUA_BUDDY_STATUS_ONLINE:
                presStatus = SipManager.PresenceStatus.ONLINE;
                break;
            case PJSUA_BUDDY_STATUS_OFFLINE:
                presStatus = SipManager.PresenceStatus.OFFLINE;
                break;
            case PJSUA_BUDDY_STATUS_UNKNOWN:
            default:
                presStatus = SipManager.PresenceStatus.UNKNOWN;
                break;
        }
        // Now get infos from RPID
        switch (binfo.getRpid().getActivity()) {
            case PJRPID_ACTIVITY_AWAY:
                presStatus = SipManager.PresenceStatus.AWAY;
                if (isDefaultTxt) {
                    presStatusTxt = "";
                }
                break;
            case PJRPID_ACTIVITY_BUSY:
                presStatus = SipManager.PresenceStatus.BUSY;
                if (isDefaultTxt) {
                    presStatusTxt = "";
                }
                break;
...
//                binfo.getMonitor_pres(), presStatus, presStatusTxt);
    ...
    public void on_reg_state(final int accountId) {
        lockCpu();
        pjService.service.getExecutor().execute(new SipService.SipRunnable() {
            @Override
                    ...
                    switch (callState) {
                        ...
                        case SipCallSession.InvState.DISCONNECTED:
                            ...
                            if (stateReceiver.mIntegrateWithCallLogs) {
                                ...
                                SipUri.ParsedSipContactInfos callerInfos = SipUri.parseSipContact(cv
